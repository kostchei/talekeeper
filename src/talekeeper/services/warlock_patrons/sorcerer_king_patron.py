...
class SorcererKingPatron:
    ...
    def use_voice_of_tyranny(self, character_id: str) -> bool:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                WHERE character_id = ? AND feature_name = 'Voice of Tyranny'
            """, (character_id,))
            ...
            current_uses = mechanics.get('uses_current', charisma_mod)

            if current_uses <= 0:
                return False

            # Use the feature
            mechanics['uses_current'] = current_uses - 1
            cursor.execute("""
    ...
    def use_decisive_edict(self, character_id: str, targets: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                SELECT mechanics FROM character_features
                WHERE character_id = ? AND feature_name = 'Decisive Edict'
            """, (character_id,))
    ...
    def use_vindictive_rebuke(self, character_id: str, attacker_name: str = "attacker") -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                SELECT mechanics FROM character_features
                WHERE character_id = ? AND feature_name = 'Vindictive Rebuke'
            """, (character_id,))
    ...
    def enhance_command_spell(self, character_id: str, base_targets: int = 1) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                SELECT mechanics FROM character_features
                WHERE character_id = ? AND feature_name = 'Absolute Tyranny'
            """, (character_id,))
    ...
    def long_rest_recovery(self, character_id: str):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                UPDATE character_features
                SET mechanics = json_set(mechanics, '$.uses_current', ?)
                WHERE character_id = ? AND feature_name = 'Voice of Tyranny'
    ...
    def get_patron_features(self, character_id: str) -> List[Dict[str, Any]]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                SELECT feature_name, description, usage_type, mechanics FROM character_features
                WHERE character_id = ? AND mechanics LIKE '%warlock_patron%'
            """, (character_id,))

            features = []
            for row in cursor.fetchall():
                feature_name, description, usage_type, mechanics_json = row
